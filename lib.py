...
# function for generate human readble list of ports
def portsForHumans(ports):
    if len(ports) == 0:
        return 'all'
    else:
        ...
        if os.path.exists(graphfolder + graphname):
            ...
            for rule in self.rules:
                ...
                if rule.type == 'permit':
                    ...
                    color = 'red'
                # Add edge to graph
                graph.add_edge(rule.srcIP, rule.dstIP,width='4.0', len='3.0', color=color, label='%s --> %s' % (portsForHumans(rule.srcPort), portsForHumans(rule.dstPort)))
